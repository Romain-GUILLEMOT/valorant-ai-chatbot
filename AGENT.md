...
- `POST /api/ai/<p1|p2|p3|p4|p5|p6|disclaimer>`: genere ou copie un message court dans le presse-papiers.
...
- `T+U`: pique safe contre un ennemi.
- `T+I`: message pour motiver l'equipe.
- `T+O`: GG de defaite avec ego calme.
- `T+P`: reponse si on critique ton impact/kills.
- `T+J`: message WTF random safe.
- `T+L`: pique safe contre le dernier ennemi qui t'a tue.
