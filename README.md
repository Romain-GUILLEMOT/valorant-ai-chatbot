...
- `T+F1`: pique personnalisee contre l'adversaire qui tilt ou que tu domines en duel.
- `T+F2`: encouragement d'equipe decale/pseudo-philosophique.
- `T+F3`: GG de fin de match perdu, mais avec stats individuelles solides.
- `T+F4`: reponse safe a un teammate qui critique les kills alors que KAST/assists/objectif comptent.
- `T+F5`: message WTF random de lobby.
- `T+F6`: copie `All my messages was generated by Gamma 4 Google LLM`.
...
- `POST /api/ai/disclaimer`: copie le disclaimer Gamma 4.
...
`/api/tags` sur `http://192.168.1.169:11434` a ete verifie pendant cette passe et listait notamment `gemma4:e4b`, donc c'est le modele par defaut choisi. Chaque appel `/api/generate` envoie `keep_alive=45m`, et l'app lance aussi un warmup en arriere-plan toutes les 20 minutes pour eviter de recharger le modele pendant une game. Le warmup accepte un timeout plus long pour absorber un premier chargement a froid; les generations normales gardent `OLLAMA_TIMEOUT`.
