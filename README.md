...
- `T+1`: pique personnalisee contre l'adversaire qui tilt ou que tu domines en duel.
- `T+2`: encouragement d'equipe decale/pseudo-philosophique.
- `T+3`: GG de fin de match perdu, mais avec stats individuelles solides.
- `T+4`: reponse safe a un teammate qui critique les kills alors que KAST/assists/objectif comptent.
- `T+5`: message WTF random de lobby.
- `T+G`: copie `All my messages was generated by Gamma 4 Google LLM`.
